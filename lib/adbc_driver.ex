defmodule Adbc.Driver do
  ...
  def download(driver_name, opts) when driver_name in @official_drivers and is_list(opts) do
    ...
    timeout = opts[:timeout] || :timer.seconds(120)
    ...
    with {:ok, triplet} <- current_triplet(),
         ...
         {:ok, cache_path} <- cached_download(url, ignore_proxy, timeout, driver_name, version, triplet) do
  ...
  defp cached_download(url, ignore_proxy, timeout, driver_name, version, triplet) do
    ...
    if File.exists?(cache_path) do
      ...
      with {:ok, body} <- Helper.download(url, ignore_proxy, timeout) do
