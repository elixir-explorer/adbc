...
:ok = Statement.set_sql_query(statement, "CREATE TABLE IF NOT EXISTS foo (i64 INT, f64 REAL)")
:ok = Statement.prepare(statement)
{:ok, _stream, _row_affected} = Statement.execute_query(statement)

:ok = Statement.set_sql_query(statement, "INSERT INTO foo(i64,f64) VALUES(?,?)")
:ok = Statement.prepare(statement)
r = :random.uniform(1000)
# :ok = Statement.bind(statement, [r, r*1.1, "value = #{inspect(r)}"])
:ok = Statement.bind(statement, [r, r*1.1])
